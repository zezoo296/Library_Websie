...
async function getShelfData(category, token) {
...
const MyLibraryPage = () => {
    ...
    const { data, isLoading, isError, error } = useQuery({
        ...
        queryFn: () => getShelfData(category, token),
        enabled: !!token
    })
        ...
        <>
            ...
            {!isLoading && data?.length > 0 && (
                <div className={classes.books}>
                    {data.map(book =><div className='flex flex-col justify-center items-center w-full gap-3'>
